...
with open("productos.csv", "r") as productos: #Abre el archivo productos.csv en modo read
        Pcaro = productoMasCaro(productos) #Guarda en Pcaro el resultado de ocupar la funcion productoMasCaro con el archivo

with open("productos.csv", "r") as productos: #Abre el archivo productos .csv en modo read
        VTotal = valorTotalBodega(productos) #Guarda en VTotal el resultado de ocupar la funcion valorTotalBodega

with open("items.csv", "r") as items, open("productos.csv", "r") as productos: #Abre los archivos items.csv y productos.csv
        PMIngresos = productoConMasIngresos(items, productos) #Ocupa la funcion productoConMasIngresos en los archivos y guarda su resultado en PMIngresos

with open("items.csv", "r") as items, open("productos.csv", "r") as productos, open("ventas.csv", "r") as ventas: ##Abre los archivos items.csv, productos.csv y ventas.csv
        TotalV = totalVentasDelMes(2010, 10, items, productos, ventas) #Ocupa la funcion totalVentasDelMes y guarda el resultado en TotalV


with open("informe.txt", "w", encoding="utf-8") as informe: #Abre o crea el archivo informe.txt en modo write
        informe.write(f" El producto más caro es {Pcaro}\n")                              #
        informe.write(f" El valor total de la bodega es de ${int(VTotal)}\n")             #
        informe.write(f" El producto con más ingresos es {PMIngresos}\n")                 #
        informe.write(f" En el período de 10/2010, el total de ventas es de ${TotalV}\n") # Ordena los datos en el archivo
with open("informe.txt", "r") as informe: #Abre el archivo informe.txt en modo read
    print(informe.read()) #Muestra en pantalla la lectura del archivo
